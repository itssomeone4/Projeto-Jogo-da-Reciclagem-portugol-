programa
{
	...
	inteiro derrota_jogo_spr = g.carregar_imagem(pasta_sprites+"derrota_jogo_spr.jpeg")
	inteiro lixos_spr = g.carregar_imagem(pasta_sprites+"lixos_spr.jpeg")
	inteiro vitoria_fase_joia_spr = g.carregar_imagem(pasta_sprites+"vitoria_fase_joia_spr.jpeg")
	...
	funcao fase_1(){
		real velocidade = 0.5
		lixosCaindo(velocidade,bolinha_papel_spr,lixo_azul_spr)
	}
	funcao fase_2(){
		real velocidade = 0.75
		lixosCaindo(velocidade,garrafa_plastico_spr,lixo_vermelho_spr)
	}
	funcao fase_3(){
		real velocidade = 1.0	
		lixosCaindo(velocidade,garrafa_vidro_spr,lixo_verde_spr)
	}
	funcao fase_4(){
		real velocidade = 1.25
		lixosCaindo(velocidade,lata_metal_spr,lixo_amarelo_spr)
	}
	funcao lixosCaindo(real velocidade,inteiro lixo_spr,inteiro lata_spr){
		enquanto(qtd_lixo > 0){
	...
	funcao tela_vitoria_fase(){
		...
		g.limpar()
		g.desenhar_imagem(400, 400,vitoria_fase_joia_spr)
		g.renderizar()
	}
